#!/usr/bin/env python3
"""M4b: R*=2.76 theta_eq sweep with low amp.
R*=2.76: amp has NO effect (k=1.6857 invariant across amp=0.3-1.0).
The issue is Dy too small (35 vs expected ~44 for k=1.33).
Lower theta_eq may reduce contact line pinning and increase Dy.

Target: k ≈ 1.33 ± 0.15 (Liu 2015)
"""
...
print(f"M4b: R*=2.76 theta_eq sweep (amp=0.5)")
print(f"Target k = 1.33 ± 0.15 (Liu 2015)")
print("=" * 60)
...
theta_values = [140.0, 150.0, 155.0, 158.0, 160.0, 162.0]
amp_fixed = 0.5
results = []
...
for theta in theta_values:
    ...
    solver = AllenCahnSolver(
        ...
        stab_mode='fakhari',
        boundary_relax=0.0,
        geometric_wetting=(amp_fixed > 0),
        geo_amplification=amp_fixed
    )
    ...
    in_range = abs(max_k - 1.33) <= 0.15 if stable else False

    print(f"theta={theta:.0f}° amp={amp_fixed:.1f} | k={max_k:.4f} Dx={Dx_peak:.0f} Dy={Dy_peak:.0f} "
          f"err={err_pct:+.1f}% {'✓' if in_range else '✗'} [{tag}] ({elapsed:.0f}s)")

    results.append({
        'theta_eq': theta, 'amp': amp_fixed, 'max_k': max_k,
        'max_k_step': max_k_step, 'Dx': Dx_peak, 'Dy': Dy_peak,
        'stable': stable, 'elapsed_s': round(elapsed, 1),
        'error_pct': round(err_pct, 1), 'in_range': in_range, 'R_star': R_star
    })

    del solver

print("\n" + "=" * 60)
print("M4b R*=2.76 THETA_EQ SWEEP RESULTS")
print("=" * 60)
print(f"{'theta':>6} {'amp':>5} {'k':>7} {'Dx':>5} {'Dy':>5} {'err%':>7} {'in_range':>8}")
print("-" * 43)
for r in results:
    mark = "✓" if r['in_range'] else " "
    print(f"{r['theta_eq']:6.0f}° {r['amp']:5.1f} {r['max_k']:7.4f} "
          f"{r['Dx']:5.0f} {r['Dy']:5.0f} {r['error_pct']:7.1f}% {mark:>8}")

save_path = '/mnt/qmingjun/pytorch_lbm/results/m4b_r276_theta_sweep.json'
...
if valid:
    best = min(valid, key=lambda r: abs(r['max_k'] - 1.33))
    print(f"\nBest: theta={best['theta_eq']}° -> k={best['max_k']:.4f} (err={best['error_pct']:.1f}%)")
else:
    all_valid = [r for r in results if r['stable']]
    if all_valid:
        closest = min(all_valid, key=lambda r: abs(r['max_k'] - 1.33))
        print(f"\nNo configs in ±0.15 range. Closest: theta={closest['theta_eq']}° -> k={closest['max_k']:.4f}")