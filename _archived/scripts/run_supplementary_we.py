#!/usr/bin/env python3
"""Supplementary We sweep densification + morphology snapshots (12 cases).

Machine: GTX 1080 8GB (CUDA)
Purpose: Fill gaps in We sweep curve (ridge R*=1.0, θ=162°) and capture
         droplet morphology snapshots for thesis figures.

Cases:
  - We = 4, 5, 6, 8, 9, 10, 14, 15, 17, 20, 25, 30
  - All: ridge R*=1.0, θ=162°, amp=1.8, 150²×nz, 3000 steps
  - All: save 10 morphology snapshots each

We=5,10,15,20 are re-runs (k data exists from s3a) for snapshot capture.
We=4,6,8,9,14,17,25,30 are entirely new data points.

Estimated: 12 × ~15min = 3h
Usage: python3 scripts/run_supplementary_we.py
"""
import sys
import os
...
def run_case(We=7.9, theta_eq=162.0, amp=1.8, N=3000, n_base=150,
    ...
    if label is None:
        label = f"sup_We{We:.1f}"

    tag = "OK" if stable else "FAIL"
    print(f"  {label:>30} We={We:5.1f} | "
    ...
    return {
        ...
        'machine': 'remote_gtx1080', 'k_history': k_history,
    }
...
def main():
    ...
    we_values = [4, 5, 6, 8, 9, 10, 14, 15, 17, 20, 25, 30]

    print("\n" + "=" * 70)
    print("  SUPPLEMENTARY: We sweep densification + morphology snapshots")
    print(f"  {len(we_values)} cases: We={we_values}")
    print("=" * 70)

    for we in we_values:
        label = f"morph_We{we:.1f}" if we in [5, 10, 15, 20] else f"sup_We{we:.1f}"
        r = run_case(We=we, theta_eq=162.0, amp=1.8, N=3000, n_base=150,
                     label=label, save_snapshots=True)
        results.append(r)

    # Summary
    print("\n" + "=" * 70)
    print("  SUPPLEMENTARY WE SWEEP RESULTS")
    print("=" * 70)
    ok = sum(1 for r in results if r['stable'])
    print(f"\n  Total: {len(results)} cases, {ok} OK, {len(results)-ok} FAIL")
    for r in results:
        tag = "OK" if r['stable'] else "FAIL"
        print(f"  {r['label']:>30} We={r['We']:5.1f} k={r['max_k']:.4f} [{tag}]")

    # Save results
    os.makedirs(RESULTS_DIR, exist_ok=True)
    save_path = os.path.join(RESULTS_DIR, 'supplementary_we_results.json')
    ...
    # Save history
    history_path = os.path.join(RESULTS_DIR, 'supplementary_we_history.json')
