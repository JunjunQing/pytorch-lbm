#!/usr/bin/env python3
"""Background run: Ridge method comparison at N=150.

Three cases: amp=0.0, 1.0, 1.5
Used for improved Phase 2.3 figure.
"""
...
def measure_droplet(phi_np, solid_np, cx, cy):
    ...
    return {'Dx': Dx, 'Dy': Dy, 'Dz': Dz, 'k': k,
            'spread_radius': float(dr.max()), 'z_max': float(coords[:, 2].max()),
            'volume': volume}
...
def run_ridge(amp):
    ...
    return {
        'amp': amp, 'geometry': 'ridge', 'grid': f'{nx}x{ny}x{nz}',
        'final': history[-1] if history else {}, 'max_k': max_k,
...
for amp in amps:
    ...
    results.append(r)
    f = r['final']
    print(f"  k_max={r['max_k']:.4f}, final_k={f.get('k',0):.4f}")
    ...
    print(f"  volume={f.get('volume',0):.1f} [{r['elapsed_s']}s, {r['mem_mb']:.0f}MB]")

print("\n" + "="*60)
print("RIDGE N=150 COMPARISON")
print("="*60)
for r in results:
    f = r['final']
    print(f"  amp={r['amp']:.1f}: k_max={r['max_k']:.4f}, "
          f"final_k={f.get('k',0):.4f}, "
          f"D={f.get('Dx',0):.0f}×{f.get('Dy',0):.0f}×{f.get('Dz',0):.0f}")

save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         '..', 'results', 'phase4_ridge_n150_comparison.json')
