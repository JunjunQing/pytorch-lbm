#!/usr/bin/env python3
"""Phase 3: Convex hemisphere generalization of geo_amplification.

Tests convex geometry with amp=0 and amp=1.5.
Key metric: droplet shape (height, spread radius, aspect ratio)
on an axisymmetric curved surface.
"""
...
print(f"\nGrid: {nx}x{ny}x{nz}, R_drop={R_drop:.0f}, R_g={R_g:.0f}")
print(f"sigma={sigma:.6f}, beta={beta:.6f}, kappa={kappa:.6f}, M={M:.6e}")
...
for amp in [0.0, 1.5]:
    ...
    print(f"  final: Dx={f.get('Dx',0):.0f} Dy={f.get('Dy',0):.0f} Dz={f.get('Dz',0):.0f}")
    print(f"  aspect={f.get('aspect_ratio',0):.4f} k={f.get('k',0):.4f}")
    print(f"  spread_radius={f.get('spread_radius',0):.1f} z_max={f.get('z_max',0):.1f}")
    print(f"  volume={f.get('volume',0):.1f} [{tag}] ({r['elapsed_s']}s, {r['mem_mb']:.0f}MB)")

print("\n" + "="*60)
print("PHASE 3 CONVEX RESULTS")
print("="*60)
print(f"{'amp':>5} {'aspect':>8} {'k':>6} {'spread_R':>10} {'z_max':>6} "
      f"{'Dx':>5} {'Dy':>5} {'Dz':>5} {'vol':>8}")
print("-"*65)
for r in results:
    f = r['final']
    print(f"{r['amp']:5.1f} {f.get('aspect_ratio',0):8.4f} "
          f"{f.get('k',0):6.4f} {f.get('spread_radius',0):10.1f} "
          f"{f.get('z_max',0):6.0f} {f.get('Dx',0):5.0f} "
          f"{f.get('Dy',0):5.0f} {f.get('Dz',0):5.0f} "
          f"{f.get('volume',0):8.1f}")

# Save
save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         '..', 'results', 'phase3_convex_study.json')
...
# Summary
if len(results) >= 2:
    a0 = results[0]['final'].get('aspect_ratio', 0)
    a1 = results[1]['final'].get('aspect_ratio', 1)
    print(f"\nAspect ratio change: {a0:.4f} -> {a1:.4f} ({((a1-a0)/a0*100):+.1f}%)")
    s0 = results[0]['final'].get('spread_radius', 0)
    s1 = results[1]['final'].get('spread_radius', 0)
    print(f"Spread radius change: {s0:.1f} -> {s1:.1f} ({((s1-s0)/s0*100):+.1f}%)")
    v0 = results[0]['final'].get('volume', 0)
    v1 = results[1]['final'].get('volume', 0)
    print(f"Volume: {v0:.0f} -> {v1:.0f} ({(v1/v0-1)*100:+.2f}%)")
